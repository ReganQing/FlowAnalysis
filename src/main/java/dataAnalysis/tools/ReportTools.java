...
public class ReportTools {
    ...
    public String generateSalesReport(
            @P("数据概况") String dataOverview,
            @P("趋势分析结果") String trendAnalysis,
            @P("产品排行结果") String productRanking,
            @P("区域分析结果") String regionalAnalysis,
            @P("渠道分析结果") String channelAnalysis,
            @P("图表路径列表") List<String> chartPaths,
            @P("核心发现") String keyFindings,
            @P("业务建议") String recommendations) throws IOException {
        ...
        // 执行摘要
        report.append("## 执行摘要\n\n");
        report.append(keyFindings).append("\n\n");

        // 数据概况
        report.append("## 1. 数据概况\n\n");
        report.append(dataOverview).append("\n\n");

        // 趋势分析
        report.append("## 2. 销售趋势分析\n\n");
        report.append(trendAnalysis).append("\n\n");

        // 产品排行
        report.append("## 3. 产品销售排行\n\n");
        report.append(productRanking).append("\n\n");

        // 区域分析
        report.append("## 4. 区域销售分析\n\n");
        report.append(regionalAnalysis).append("\n\n");

        // 渠道分析
        report.append("## 5. 销售渠道对比\n\n");
        report.append(channelAnalysis).append("\n\n");

        // 数据可视化
        if (chartPaths != null && !chartPaths.isEmpty()) {
            report.append("## 6. 数据可视化\n\n");
            for (int i = 0; i < chartPaths.size(); i++) {
                report.append(String.format("### 图表 %d\n\n", i + 1));
                // 转换路径为相对路径
                String relativePath = chartPaths.get(i).replace("output/", "../output/");
                report.append(String.format("![%s](%s)\n\n", "Chart " + (i + 1), relativePath));
            }
        }

        // 业务建议
        report.append("## 7. 业务建议\n\n");
        report.append(recommendations).append("\n\n");

        // 页脚
        report.append("---\n\n");
        report.append("*本报告由 LangChain4j 数据分析多智能体系统自动生成*\n");

        // 保存报告
        String filename = String.format("sales_analysis_report_%s.md", TIMESTAMP.format(LocalDateTime.now()));
        String filepath = OUTPUT_DIR + filename;
